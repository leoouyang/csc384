...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.beliefs.sortedKeys():
            if self.beliefs[p] != 0:
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            self.particles.append(self.legalPositions[i%legalNum])
        "*** END YOUR CODE HERE ***"
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for i in range(self.numParticles):
                newParticles.append(jail)
            self.particles = newParticles
        else:
            WeightedPos = util.Counter()
            for p in self.particles:
                trueDistance = util.manhattanDistance(p,pacmanPosition)
                weight = emissionModel[trueDistance]
                WeightedPos[p] += weight

            if WeightedPos.totalCount() > 0:
                for i in range(self.numParticles):
                    newParticles.append(util.sample(WeightedPos))
                self.particles = newParticles
            else:
                self.initializeUniformly(gameState)
        "*** END YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        possibleNum = len(possible)
        for i in range(self.numParticles):
            self.particles.append(possible[i%possibleNum])
        "*** END YOUR CODE HERE ***"
    ...
    def observeState(self, gameState):
        ...
        for p in self.particles:
            ...
            for GIndex in range(self.numGhosts):
                if noisyDistances[GIndex] is not None:
                    trueDistance = util.manhattanDistance(p[GIndex],pacmanPosition)
                    weight *= emissionModels[GIndex][trueDistance]
            WeightedPos[p] += weight
